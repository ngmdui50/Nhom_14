...
class LoginApp(QMainWindow):
    ...
    def __init__(self):
        ...
        try:
            ...
            print(f"Loi load form Dang nhap: {e}")
    ...
    def check_login(self):
        user = authenticate(self.txtusername.text().strip(), self.txtpassword.text().strip())
        if not user:
            QMessageBox.warning(self, "Loi dang nhap", "Ten dang nhap hoac mat khau khong dung!")
            return
...
class DialogChiTietLichKham(QDialog):
    ...
    def init_ui(self):
        uic.loadUi("View/FormChiTietLichKham.ui", self)
        ...
        try:
            ...
            for x in lk_data:
                if str(val(x, "id", "ID")).strip() == str(self.lich_id).strip():
                    ...
                    break
            if not l:
                raise Exception(f"Khong tim thay lich kham co ID {self.lich_id}")
            ...
            self.txtbenhnhan.setText(dict_bn.get(str(val(l, "benh_nhan_id", "benhnhan_id")), "Khong ro"))
            self.txtbacsi.setText(dict_bs.get(str(val(l, "bac_si_id", "bacsi_id")), "Khong ro"))
            self.txtdichvu.setText(dict_dv.get(str(val(l, "dich_vu_id", "dichvu_id")), "Khong ro"))
            ...
            QMessageBox.critical(self, "Loi lay du lieu", str(e))
            self.close()
        ...
        try:
            ...
            if tt_new != tt_old:
                mo_ta_new, ok = QInputDialog.getMultiLineText(
                    self,
                    "Mo ta chuyen trang thai",
                    f"Nhap mo ta/ly do chuyen trang thai tu '{tt_old}' sang '{tt_new}':",
                    str(getattr(self.lich_hien_tai, "mo_ta", "") or ""),
                )
                if not ok:
                    return
            if QMessageBox.question(self, "Xac nhan", "Ban co chac chan muon luu thong tin da sua?", QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No) == QMessageBox.StandardButton.Yes:
                LichKhamController.update_tu_form_chi_tiet(self.lich_id, ngay_new, gio_new, tt_new, mo_ta_new)
                QMessageBox.information(self, "Thanh cong", "Da cap nhat thong tin lich kham thanh cong!")
                if hasattr(self, "main_app") and self.main_app:
                    self.main_app.dong_bo_hoa_don_theo_trang_thai(self.lich_id, tt_new, ngay_new)
                    self.main_app.hien_thi_timeline()
                self.close()
        except Exception as e:
            QMessageBox.critical(self, "Loi khi luu", str(e))
...
class ThemBenhNhanWindow(QDialog):
    def __init__(self, parent_main, preset_name=""):
        ...
        try:
            ui_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "View", "FormThemBenhNhan.ui")
            ...
            print(f"Khong the load file FormThemBenhNhan.ui: {e}")

    def xu_ly_luu(self):
        ten = self.txtTenBN.text().strip()
        if not ten:
            QMessageBox.warning(self, "Loi", "Vui long nhap ten benh nhan!")
            return
        ...
        try:
            ...
            QMessageBox.information(self, "Thanh cong", f"Da them benh nhan: {ten}")
            ...
            QMessageBox.critical(self, "Loi", f"Khong the luu: {e}")
